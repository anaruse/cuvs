    ...
    for ( int batch_size : batch_size_list ) {
        for ( int internal_topk : internal_topk_list ) {
            ...
            if (algo == "single-cta") {
                ...
                //
                // (*) The following itopk size adjustment according to filtering
                // rate is no longer necessary, as it has beeen integrated to cuVS.
                //
                // _internal_topk = (topk / bitset_rate) + (_internal_topk - topk) / std::sqrt(bitset_rate);
                // _internal_topk = (_internal_topk / 32) * 32;
                //
                // (*) If you want do disable automatic search parameters
                // adjustment according to filtering rate, set filtering rate to zero.
                //
